...
export async function processConversationSlice(
  ...
  try {
    ...
    const dialogueContext = dialogueHistory.map(line => `${line.role === "user" ? "User" : "Shefali"}: ${line.text}`).join("\n");
...
- ID: For ADD, leave blank. For UPDATE or REMOVE, provide the exact 'id' from the "Current user memories" list.`;
    ...
    // PRUNING: Prevent infinite memory bloat by keeping only the 50 most recently updated memories.
    updatedMemories.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
    if (updatedMemories.length > 50) {
      updatedMemories = updatedMemories.slice(0, 50);
    }
